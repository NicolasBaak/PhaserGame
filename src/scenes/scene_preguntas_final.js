...
class Scene_preguntas_final extends Phaser.Scene{
    ...
    if (Math.floor(score) <= 30) {
        let scoreText = this.add.text(center_width, 440, 'No te preocupes por el resultado ' +  + ', recuerda que la práctica hace al maestro!! 🦾', { fontFamily: 'Minecraft, Georgia, "Goudy Bookletter 1911", Times, serif', fontSize: '32px'});
        scoreText.setOrigin(0.5, -2.5);
    } else if (Math.floor(score) <= 66) {
        let scoreText = this.add.text(400, 300, 'Lo hiciste bien ' + 'Nicolas' + '! , pero recuerda que siempre puedes mejorar 😎', { fontFamily: 'Minecraft, Georgia, "Goudy Bookletter 1911", Times, serif', fontSize: '32px'});
        scoreText.setOrigin(0.5, -2.5);
    } else if (Math.floor(score) <= 80) {
        let scoreText = this.add.text(400, 300, 'Muy bien ' + 'Nicolas' + '!! Tu resultado refleja que tienes buenos conocimientos 😋', { fontFamily: 'Minecraft, Georgia, "Goudy Bookletter 1911", Times, serif', fontSize: '32px'});
        scoreText.setOrigin(0.5, -2.5);
    } else {
        let scoreText = this.add.text(400, 300, 'Excelente ' + 'Nicolas' + ', muchas felicidades!!! 🥇, el esfuerzo de tu estudio se nota', { fontFamily: 'Minecraft, Georgia, "Goudy Bookletter 1911", Times, serif', fontSize: '32px'});
        scoreText.setOrigin(0.5, -2.5);
    }

    const restartButton = new Button( this, 150, center_height*2-50, 'button-menu', 'button-menu-hover').setScale(0.5);
